...
depends_on: [T-000]
touches: [<path/ไฟล์.ts>, <โฟลเดอร์/>]   # ไฟล์/โฟลเดอร์ที่คาดว่าจะแก้ — /spec ใช้ตัดสิน [P] · /task เทียบกับ PR ที่เปิดอยู่ · docs-lint เตือนเมื่อสอง task เปิดพร้อมกันแตะที่เดียวกัน
fixes: <T-xxx ที่ปิดไปแล้วซึ่งงานนี้มาแก้ — ถ้าไม่ใช่งานแก้ให้ลบบรรทัดนี้>
...
> รายการเดียวกับ `touches:` ด้านบน (ที่นี่อธิบายได้ว่าแต่ละไฟล์จะเปลี่ยนอะไร) — ถ้าเกิน ~10 ไฟล์ → แตก task
> ไฟล์เดียวกับ task อื่นที่เปิดพร้อมกันได้ → รวม task, ใส่ `depends_on:` ให้ทำทีละงาน หรือแตกใหม่ตาม feature แทนตามชั้น
