...
> **MINOR:** ไม่มีไฟล์ควบคุมเปลี่ยน · ไม่มี schema เปลี่ยน · โปรเจกต์ที่ใช้ Prettier ได้บรรทัดใหม่ใน `.prettierignore` · ไฟล์ที่ lock รับไว้แต่ kit ไม่เคยส่งกลายเป็น conflict

**ไฟล์ของ kit อยู่ตามต้นฉบับ Prettier ทำให้ lock โกหกไม่ได้อีก** (PE-012) — จากโปรเจกต์จริง: lint-staged รัน `prettier --write`
กับ `.claude/*.js` ทุก commit ไฟล์ของ kit ทั้ง 28 ไฟล์เลยไม่ตรงกับ lock · session อัปเกรดอ่านผิดว่าเป็นไฟล์รุ่น 3.13.1 ที่ค้าง
แล้ว `lock --write` รับไฟล์ที่ถูก format ไป · lock บันทึกสิ่งที่อยู่บนดิสก์ ไม่ใช่ว่าเป็นของใคร ⇒ patch ของ `docs-lint.js`
ที่ทีมแก้เองถูก install รอบถัดไปทับแบบ 0 conflict

- **`install` ถามประวัติ kit ก่อน lock** — ทับเงียบ ๆ เฉพาะเนื้อหาที่ kit รุ่นไหนสักรุ่นเคยส่ง · ตรงกับ lock แต่ kit ไม่เคยส่ง
  = `conflict` ที่บอกว่ารับเข้า lock มาและเอาของ kit ได้ด้วย `--force`
- **โปรเจกต์ที่ใช้ Prettier ได้ไฟล์ของ kit ใน `.prettierignore`** — ครั้งเดียว ต่อท้ายของเดิม · ไม่แตะถ้า ignore `.claude/` ทั้งโฟลเดอร์อยู่แล้ว
- **`doctor` เตือน formatter ที่จะเขียนทับไฟล์ของ kit** และคำเตือน drift ไม่บอกให้ `lock --write` อีก
- **`buaflow upgrade` รายงาน `formatter`** และ `/buaflow:upgrade` อธิบาย conflict แบบ `accepted into the lock`

---

## v3.18.0 — 2026-09-26

> **MINOR:** ไม่มีไฟล์ในโปรเจกต์ที่ต้องแก้มือ · ไม่มี schema เปลี่ยน · คำสั่ง `buaflow upgrade` และ skill `/buaflow:upgrade` ใหม่

**อัปเกรดเป็นคำสั่งเดียว รายงานเดียว จากรุ่นไหนก็ได้** (PE-011) — `/buaflow:start` เคยทั้งเปิดโปรเจกต์และอัปเกรด ทางอัปเกรดให้ AI
อ่าน UPGRADE.md พันบรรทัดกับตารางสัญญาณใน START-HERE.md แล้วไล่เช็คเงื่อนไขเอง ⇒ ช้า กิน token · และระหว่างตรวจพบบั๊กจริง:
โปรเจกต์ที่ไม่มี lock อัปเกรดจาก plugin แล้วไฟล์ kit รุ่นเก่าทุกไฟล์ขึ้น conflict เพราะ kit ใน plugin cache ไม่ใช่ git checkout

- **`buaflow upgrade [--plugin] [--write] [--force]`** — รายงานเดียวจากไฟล์ของโปรเจกต์: รุ่นที่ติดตั้ง (lock · เนื้อหาไฟล์ · หรือเดา
  จากไฟล์ที่มี) · ทางลัดหรือไล่ทีละรุ่นพร้อมหัวข้อที่ต้องเริ่ม · ไฟล์ที่จะเปลี่ยน · ขั้นที่ต้องลงมือเฉพาะข้อที่เข้าเงื่อนไข · `--plugin`
  แยกสำเนา kit ใน `.claude/` ที่ลบได้ ออกจากไฟล์ที่ทีมแก้และไฟล์ของทีมเอง · `--write` = `install --write` ไม่ลบอะไร
  และไม่ยอมเขียนทับโปรเจกต์ที่ติดตั้งจาก kit ที่ใหม่กว่า
- **`/buaflow:upgrade`** — skill ใหม่ใน plugin ทำงานจากรายงานนั้น อ่าน UPGRADE.md เฉพาะหัวข้อที่รายงานบอก
- **`/buaflow:start` ไม่อัปเกรดแล้ว** และไม่รัน `doctor` เพื่อหาสถานะ — ใช้บรรทัดที่ hook ส่งมา · โปรเจกต์ตามหลัง → บอกหนึ่งบรรทัดแล้วทำต่อ
- **`claude-setup/kit-history.json`** — hash ของทุกเนื้อหาที่ kit เคยติดตั้ง (49 ไฟล์ 185 เวอร์ชัน 18 KB) มากับ kit · `install`
  และ `lock` ใช้แยกไฟล์ kit รุ่นเก่าออกจากไฟล์ที่ทีมแก้ โดยไม่ต้องพึ่ง git · โปรเจกต์จาก kit 3.9.0: conflict ปลอม 10 → 0
- **hook ตอนเปิด session ไม่นับ `.claude/skills/` ของทีมเองเป็นการติดตั้ง Buaflow** — นับเฉพาะ skill ที่ kit แจก
