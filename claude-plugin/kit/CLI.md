...
| `usage consent --enable\|--disable` · `status` · `record check --task <id> --verdict pass\|fail` · `setup --store <clone>` · `sync` · `report [--out <file>] [--since <date>]` · `show <project>/<task>` · `eval-draft --task <project>/<task> [--out <dir>]` | เก็บข้อมูลการใช้งานภายในแบบถามยินยอมก่อน (EV-011) · `consent` ตอบครั้งเดียวต่อโปรเจกต์ · ไม่ยินยอม = ไม่บันทึก ไม่ sync · `setup` ชี้ clone ของ private repo กลาง (ต่อเครื่อง) · `sync` ส่ง event ที่ยังไม่ได้ส่ง (exit 3 ถ้า push ไม่ได้ event ยังอยู่ครบ) · `report` / `show` / `eval-draft` อ่านที่เก็บกลาง ใช้ใน repo Buaflow เท่านั้น | `consent` เขียน `.buaflow/usage.json` · `record` เขียน `.buaflow/usage/` (ignore ตัวเอง) · `setup` / `report` เขียน `~/.buaflow/usage.json` · `sync` commit + push ใน clone ของที่เก็บกลาง · `report --out` / `eval-draft` เขียนไฟล์ผลลัพธ์ |
